	...
	@media screen and ( max-width: 782px ) {
		...
		* html body { margin-top: 46px !important; }
		...
		<header id="home" class="header" itemscope="itemscope" itemtype="http://schema.org/WPHeader" style="opacity: 1; min-height: 76px;">
			...
			<div id="main-nav" class="navbar" style="min-height: 30px;">
...
<div id="content" class="site-content">
	...
	<header class="entry-header">
		...
		<h1 class="entry-title" itemprop="headline">Fiche de profil</h1>		
	</header><!-- .entry-header -->
...
?>
<!--Classement des joueuse; on affiche un tableau avec le nom, score, et nbre total des parties pour chaque joueuse; 
de plus, on l'affiche dans un ordre décroissant pour avoir bien un classement-->
<style>
table, th, td {
...
<?php 	
//on trouve les pseudos, somme du score, et nbre des parties pour chaque pseudo
$requete="select ID_exp, exp_sujet, exp_theme_id, exp_lieu_nom, exp_lieu_add, exp_lieu_ville, exp_description, exp_prix, exp_nb_participants, exp_dispo_id from wp_exp";
$res=mysqli_query($connexion, $requete);


$i=1;
$j = 0;
//affichage; on chosit d' afficher que les 20 premieres joueuses, sinon, on aura un grand tableau a afficher
echo '<table>';
//	echo '<tr>';
...
<td style = "padding: 0 50px 10px 50px;" >
			<br><br>
			<h1>'.$row['exp_sujet'].'</h1></br>
			<h2>Lieu : '.$row['exp_lieu_nom'].'</h2>
			<h3>Prix : '.$row['exp_prix'].' euros</h3>
			<input type="submit" name="valider" data-target="#myModal" value="Voir l\'annonce"/></td>';
			$i++;		

}		
		//$requete1="select exp_sujet, exp_theme_id, exp_lieu_nom, exp_lieu_add, exp_lieu_ville, exp_description, exp_prix, exp_nb_participants, exp_dispo_id from wp_exp where ID_exp=\"$id_exp\"";
